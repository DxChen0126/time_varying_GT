...
for(n in 1:nselect){
  ...
  save(GTsamplelist, file = paste0("./record_sim/",  
                                   "settingchoice_", m, "_","meanexpo1","_",n, "th_","_GTsample.rda"))
  # estimate GT
  ...
  save(GT.est, file = paste0("./record_sim/",  
                             "settingchoice_", m, "_","meanexpo1", "_", n, "th_","estimatedGT.rda"))
...
save(GT_est_total, file = "GT_est_corrsims_0628.rda")

load("empirical_corr_ref.rda")

View(df.sum[[1]])

estmuLB <- matrix(unlist(lapply(GT_est_total[[1]], function(x) x$mu)), ncol = 6, nrow = 50,
                  byrow = T)
colMeans(estmuLB)

View(GT_est_total[[4]][[1]])
View(df.sum[[1]])
...
for(i in 1:4){
  estmuLB <- matrix(unlist(lapply(GT_est_total[[i]], function(x) x$muLB)), ncol = 6, nrow = 50,
                    byrow = T)
  estmuUB <- matrix(unlist(lapply(GT_est_total[[i]], function(x) x$muUB)), ncol = 6, nrow = 50,
                    byrow = T)
  estmu <- matrix(unlist(lapply(GT_est_total[[i]], function(x) x$mu)), ncol = 6, nrow = 50,
                  byrow = T)
  mu.mean <- colMeans(estmu)  
  
  estsdLB <- matrix(unlist(lapply(GT_est_total[[i]], function(x) x$sdLB)), ncol = 6, nrow = 50,
                    byrow = T)
  estsdUB <- matrix(unlist(lapply(GT_est_total[[i]], function(x) x$sdUB)), ncol = 6, nrow = 50,
                    byrow = T)
  estsd <- matrix(unlist(lapply(GT_est_total[[i]], function(x) x$sd)), ncol = 6, nrow = 50,
                  byrow = T)
  sd.mean <- colMeans(estsd)  
  
  estrhoLB <- matrix(unlist(lapply(GT_est_total[[i]], function(x) x$rhoLB)), ncol = 6, nrow = 50,
                    byrow = T)
  estrhoUB <- matrix(unlist(lapply(GT_est_total[[i]], function(x) x$rhoUB)), ncol = 6, nrow = 50,
                    byrow = T)
  estrho <- matrix(unlist(lapply(GT_est_total[[i]], function(x) x$rho)), ncol = 6, nrow = 50,
                  byrow = T)
...
df.expo1GTsim_rho_0.25 <- data.frame(
...
df.expo1GTsim_rho_0.5 <- data.frame(
...
df.expo1GTsim_rho_0.75 <- data.frame(
...
df.simsum.expo1.med <- rbind(df.expo1GTsim_rho0, df.expo1GTsim_rho_0.25, 
                             df.expo1GTsim_rho_0.5, df.expo1GTsim_rho_0.75)
df.simsum.expo1.med$rho_simset <- rep(c(0, 0.25, 0.5, 0.75), each = 6)

write.csv(df.simsum.expo1.med, file = "sim_performance_expo1_medsetting.csv")
...
######## TO DO more simulations based on different expo window and GT setting and 
### with missing expo
...
save(GT_est_total, file = "GT_est_corrsims_medGT_expo7.rda")
...
write.csv(df.simsum.expo7.med, file = "sim_performance_expo7_medsetting.csv")
