...
settings <- matrix(c(c(1, 2, 3), rep(10, 3)), 
                   ncol = 2, byrow = F)
...
save(GT_est_total, file = "GT_est_addsims_expo10_0411.rda")
...
df.longGTsim <- data.frame(
  ...
  expo10.muCIrecover = indicator.mu[[1]],
  expo10.estmu = estmu.mean[[1]],
  expo10.biasmu = bias.mu[[1]],
  expo10.widthmu = width.mu[[1]],
  expo10.sdCIrecover = indicator.sd[[1]],
  expo10.estsd = estsd.mean[[1]],
  expo10.biassd = bias.sd[[1]],
  expo10.widthsd = width.sd[[1]]
)
...
df.medGTsim <- data.frame(
  ...
  expo10.muCIrecover = indicator.mu[[2]],
  expo10.estmu = estmu.mean[[2]],
  expo10.biasmu = bias.mu[[2]],
  expo10.widthmu = width.mu[[2]],
  expo10.sdCIrecover = indicator.sd[[2]],
  expo10.estsd = estsd.mean[[2]],
  expo10.biassd = bias.sd[[2]],
  expo10.widthsd = width.sd[[2]]
)
...
df.shortGTsim <- data.frame(
  ...
  expo10.muCIrecover = indicator.mu[[3]],
  expo10.estmu = estmu.mean[[3]],
  expo10.biasmu = bias.mu[[3]],
  expo10.widthmu = width.mu[[3]],
  expo10.sdCIrecover = indicator.sd[[3]],
  expo10.estsd = estsd.mean[[3]],
  expo10.biassd = bias.sd[[3]],
  expo10.widthsd = width.sd[[3]]
)

df.shortGTsim

df.expo10.sim.sum <- rbind(df.longGTsim, df.medGTsim, df.shortGTsim)
df.expo10.sim.sum$GTsetting <- c(rep("longGT", 7), rep("mediumGT", 7), rep("shortGT", 7))

write.csv(df.expo10.sim.sum, "simulation_summary_expo10d_0412.csv")
